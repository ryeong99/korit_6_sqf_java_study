package com.study.java.study.java_study.ch21_스트림;
...
public class StreamMain {
    public static void main(String[] args) {
        ...
        List<Integer> numnerList = st1.map(n -> n * 2)                      //가공
                .peek(n -> System.out.println(n))                           //가공(기타 스트림 메소드) + peak: 스트림이 실행하는 동안 중간 점검(없어도 된다.)
                .collect(Collectors.toList());                              //최종

        numnerList.forEach(n  -> System.out.println("n: " + n));

        List<Integer> filteringNumberList = numnerList.stream()
                .map(n -> n / 2)
                .filter(n -> n % 2 == 0)
        ...
        List<Integer> sortedNumberList = numnerList.stream()
                .sorted((o1, o2) -> o2 - o1)
                .collect(Collectors.toList());
